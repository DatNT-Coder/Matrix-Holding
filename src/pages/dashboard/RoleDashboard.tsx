...
export default function RoleDashboard({ role }: { role: UserRole }) {
  ...
  const navItems = [
    ...
    { label: role === "HR" ? "Nhân sự" : "Công việc", icon: ClipboardCheck },
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 flex w-72 flex-col bg-navy-dark px-5 py-7 text-white shadow-2xl transition-transform lg:translate-x-0 ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="mt-8 space-y-2">
          {navItems.map(({ label, icon: Icon, active }) => (
            <button key={label} className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm font-semibold transition ${active ? "bg-yellow-brand text-navy shadow-lg" : "text-white/70 hover:bg-white/10 hover:text-white"}`}>
              <Icon size={19} />{label}
            </button>
          ))}
        </nav>
      ...
      <main className="lg:ml-72">
        ...
        <div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
          <section className="flex flex-col justify-between gap-6 rounded-3xl bg-gradient-to-br from-navy-dark via-navy to-blue-brand px-6 py-7 text-white shadow-card-lg sm:flex-row sm:items-end sm:px-8">
            ...
            <div className="flex flex-wrap gap-3"><button onClick={() => navigate("/tuyen-dung")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-yellow-brand px-5 py-3 text-sm font-extrabold text-navy transition hover:bg-yellow-300"><ClipboardCheck size={18} />Xem công việc</button>{(role === "DIRECTOR" || role === "HR") && <><button onClick={() => navigate("/quan-tri/tin-tuc")} className="inline-flex items-center justify-center rounded-xl border border-white/30 px-5 py-3 text-sm font-extrabold text-white transition hover:bg-white/10">Quản lý tin tức</button><button onClick={() => navigate("/quan-tri/tuyen-dung")} className="inline-flex items-center justify-center rounded-xl border border-white/30 px-5 py-3 text-sm font-extrabold text-white transition hover:bg-white/10">Đăng tin tuyển dụng</button></>}</div>
          </section>
